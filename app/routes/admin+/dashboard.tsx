...
export async function action({ request }: ActionFunctionArgs) {
  ...
  // Fetch customer and dependency counts fresh to validate constraints
  ...
  const lettersTotal =
    customer._count.PrepayLetter +
    customer._count.PostpayLetter +
    customer._count.PostpayOtherLetter

  // Relaxed rule: allow attempting delete for test-generated customers when
  // at least one category is zero (users OR providers OR submissions OR letters)
  if (!isTestGenerated) {
    return await redirectWithToast('/admin/dashboard', {
      type: 'error',
      title: 'Delete not allowed',
      description: 'Only customers auto generated by test cases may be deleted from here.',
    })
  }

  const canAttempt =
    customer._count.users === 0 ||
    customer._count.providers === 0 ||
    customer._count.submissions === 0 ||
    lettersTotal === 0
  ...
  if (!canAttempt) {
    return await redirectWithToast('/admin/dashboard', {
      ...
      description: 'Requires at least one of Users, Providers, Submissions, or Letters to be zero.',
    })
  }
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      // Submissions (events/docs cascade on delete)
      await tx.submission.deleteMany({ where: { customerId } })

      // Provider events tied to this customer
      await tx.providerEvent.deleteMany({ where: { customerId } })

      // Providers (userNpi and other relations cascade via onDelete)
      await tx.provider.deleteMany({ where: { customerId } })

      // Provider groups
      await tx.providerGroup.deleteMany({ where: { customerId } })

      // Users: detach from customer (do not delete accounts)
      await tx.user.updateMany({ where: { customerId }, data: { customerId: null, providerGroupId: null } })
  ...
  await audit.admin({
    ...
    summary: `Customer deleted: ${customer.name}`,
                      ...
                      <tr key={customer.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              {(((customer.description || '').toLowerCase().includes('auto generated by test cases')) || customer.name.startsWith('C-')) && (
                                <span
                                  className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-amber-100 text-amber-800 ring-1 ring-amber-300/60"
                                  title="Auto-generated test customer"
                                >
                                  TEST
                                </span>
                              )}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <div className="flex items-center gap-2">
                            ...
                            {(() => {
                              const isTest = (customer.description || '').toLowerCase().includes('auto generated by test cases') || customer.name.startsWith('C-')
                              if (!isTest) return null
                              const letters = customer._count.PrepayLetter + customer._count.PostpayLetter + customer._count.PostpayOtherLetter
                              const canDelete = (
                                customer._count.users === 0 ||
                                customer._count.providers === 0 ||
                                customer._count.submissions === 0 ||
                              ...
                              if (canDelete) {
                                return (
                                  <button
                                    ...
                                    title="Delete customer (requires double confirmation)"
                                  >
                              ...
                              // Disabled with tooltip: requires at least one category to be zero
                              const blockers: string[] = []
                              blockers.push(`Users: ${customer._count.users}`)
                              blockers.push(`Providers: ${customer._count.providers}`)
                              blockers.push(`Submissions: ${customer._count.submissions}`)
                              blockers.push(`Letters: ${letters}`)
                              const tip = `Cannot delete: need at least one to be zero. Current → ${blockers.join(', ')}`
                              return (
                                <button
                                  type="button"
                                  className="inline-flex items-center px-3 py-2 border border-red-200 text-red-400 bg-red-50 cursor-not-allowed opacity-60 shadow-sm text-sm font-medium rounded-md"
                                  title={tip}
                                  aria-disabled="true"
                                >
                                  <Icon name="trash" className="-ml-1 mr-2 h-4 w-4" />
