  ...
  try {
    await auditEvent.admin({
      ...
      metadata: {
        ...
        route: input.route ?? '/admin/customer-manage/:customerId/providers',
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  // Assignable users: all active, non-admin roles in this customer
  const assignableUsers = await (prisma as any).user.findMany({
    where: {
      ...
      deletedAt: null,
      roles: {
        none: { name: { in: [INTEREX_ROLES.CUSTOMER_ADMIN, INTEREX_ROLES.PROVIDER_GROUP_ADMIN] } },
      },
    },
  ...
  const providerEligibility = customer.providers.map(p => {
    ...
    let eligibleUserGroupScope: 'UNGROUPED_ONLY' | 'SPECIFIC_GROUP' = hasProviderGroup
      ? 'SPECIFIC_GROUP'
      : 'UNGROUPED_ONLY'
    ...
    if (!hasProviderGroup) {
      if (anyAssignedUngrouped) {
        ...
        groupChangeBlockReason =
          'NPI has assigned user/s who are not part of a Provider Group. Unassign them first and then assign a Provider Group.'
      }
      ...
      if (anyAssignedUngrouped) {
        userAssignReason =
          'Provider is in a group but has ungrouped users assigned (remove or add them to the group).'
      }
    }
  ...
  const providerGroupMismatches = customer.providers
    .filter(p =>
      p.userNpis.some(link => {
        const userGroup = (link as any).user.providerGroupId
    ...
    .map(p => ({
      ...
      userGroups: Array.from(
        new Set(
          p.userNpis
            .map((l: any) => l.user.providerGroupId)
            .filter((g: any): g is string => Boolean(g) && g !== p.providerGroupId),
        ),
      ),
    }))
...
export async function action({ request, params }: ActionFunctionArgs) {
    ...
    try {
      ...
      try {
        await writeAudit({
          ...
          payload: { total: (list as any)?.total, page: (list as any)?.page, pageSize: (list as any)?.pageSize },
        })
      } catch {}
  ...
  if (intent === 'create') {
    ...
    // Attempt to add in PCG (tolerate duplicates)
    try {
      ...
      if (!duplicate) {
        try {
          await logProviderEvent({ providerId: 'unknown', customerId, actorId: userId, kind: 'PCG_ADD_ERROR', message: 'PCG AddProviderNPI failed', payload: { npi, orgName, error: msg } })
        } catch {}
        try {
          await writeAudit({ userId, rolesCsv, customerId, action: 'PCG_ADD_PROVIDER_NPI', entityId: null, success: false, message: 'PCG AddProviderNPI failed', payload: { npi, customerName: orgName, error: msg } })
        } catch {}
        return data({ result: submission.reply({ formErrors: [msg || 'Failed to add NPI in PCG.'] }) }, { status: 400 })
      }
      // duplicate -> allow create locally
      try {
        await writeAudit({ userId, rolesCsv, customerId, action: 'PCG_ADD_PROVIDER_NPI', entityId: null, success: true, message: 'PCG AddProviderNPI duplicate (already present)', payload: { npi, customerName: orgName } })
      } catch {}
    }
    ...
    const created = await prisma.provider.create({
      data: { npi, name, customerId, providerGroupId, active: true },
    })

    await logProviderEvent({ providerId: created.id, customerId, actorId: userId, kind: 'PCG_ADD_ATTEMPT', message: 'PCG Add Provider NPI - api call attempted', payload: { npi, customerName: orgName } })
    await logProviderEvent({ providerId: created.id, customerId, actorId: userId, kind: 'CREATED', message: `Provider created (${npi})`, payload: { name, providerGroupId } })
    if (providerGroupId) {
      await logProviderEvent({ providerId: created.id, customerId, actorId: userId, kind: 'GROUP_ASSIGNED', message: `Assigned to group ${providerGroupId}` })
    }

    try {
      await writeAudit({ userId, rolesCsv, customerId, action: 'PROVIDER_CREATE', entityId: created.id, success: true, message: `Provider created (${npi})`, payload: { npi, name, providerGroupId } })
    } catch {}
  ...
  if (intent === 'update') {
    ...
    if (nameChanged) {
      await logProviderEvent({ providerId, customerId, actorId: userId, kind: 'UPDATED', message: `Name: "${oldName || '—'}" → "${name || '—'}"`, payload: { field: 'name', from: oldName, to: name } })
    }
    if (groupChanged) {
      await logProviderEvent({ providerId, customerId, actorId: userId, kind: newGroupId ? 'GROUP_ASSIGNED' : 'GROUP_UNASSIGNED', message: newGroupId ? `Assigned to group ${newGroupId}` : 'Unassigned from provider group', payload: { field: 'providerGroupId', from: oldGroupId, to: newGroupId } })
    }
    if (activeChanged) {
      await logProviderEvent({ providerId, customerId, actorId: userId, kind: active ? 'ACTIVATED' : 'INACTIVATED', message: active ? 'Provider activated (via edit)' : 'Provider inactivated (via edit)' })
    }
    ...
    try {
      ...
      await writeAudit({ userId, rolesCsv, customerId, action: 'PROVIDER_UPDATE', entityId: providerId, success: true, message: 'Provider updated', meta: { changed }, payload: { name, providerGroupId: newGroupId, ...(typeof active !== 'undefined' ? { active } : {}) } })
    } catch {}
  ...
  if (intent === 'toggle-active') {
    ...
    await logProviderEvent({ providerId, customerId, actorId: userId, kind: active ? 'ACTIVATED' : 'INACTIVATED', message: active ? 'Provider activated' : 'Provider inactivated' })
    try {
      await writeAudit({ userId, rolesCsv, customerId, action: 'PROVIDER_TOGGLE_ACTIVE', entityId: providerId, success: true, message: active ? 'Activated' : 'Inactivated', payload: { active } })
    } catch {}
    return redirectWithToast(`/admin/customer-manage/${customerId}/providers`, {
      type: 'success',
  ...
  if (intent === 'update-group') {
    ...
    if (newGroupId) {
      ...
      // Guard Rail: if provider currently has ungrouped assigned users and is ungrouped, block adding a group
      const anyUngroupedUsers = provider.userNpis.some(l => !l.user.providerGroupId)
      if (anyUngroupedUsers && !provider.providerGroupId) {
        try {
          await writeAudit({ userId, rolesCsv, customerId, action: 'PROVIDER_UPDATE', entityId: provider.id, success: false, message: 'Blocked group assignment: ungrouped users exist', payload: { providerId: provider.id, attemptedGroupId: newGroupId, reason: 'UNGROUPED_USER_BLOCK' } })
        } catch {}
        return redirectWithToast(`/admin/customer-manage/${customerId}/providers`, {
          type: 'error',
    ...
    await logProviderEvent({ providerId: provider.id, customerId, actorId: userId, kind: newGroupId ? 'GROUP_ASSIGNED' : 'GROUP_UNASSIGNED', message: newGroupId ? `Assigned to group ${newGroupId}` : 'Unassigned from provider group', payload: { field: 'providerGroupId', from: oldGroupId, to: newGroupId } })
    try {
      await writeAudit({ userId, rolesCsv, customerId, action: 'PROVIDER_UPDATE', entityId: provider.id, success: true, message: newGroupId ? 'Provider group assigned' : 'Provider group unassigned', payload: { providerGroupId: newGroupId }, meta: { changed: ['providerGroupId'] } })
    } catch {}
    return redirectWithToast(`/admin/customer-manage/${customerId}/providers`, {
      type: 'success',
  ...
  if (intent === 'update-user-assignment') {
    ...
    if (targetUser.roles?.some(r => [INTEREX_ROLES.CUSTOMER_ADMIN, INTEREX_ROLES.PROVIDER_GROUP_ADMIN].includes(r.name))) {
      try {
        await writeAudit({ userId, rolesCsv, customerId, action: 'PROVIDER_ASSIGN_USER_ATTEMPT', entityId: provider.id, success: false, message: 'Blocked admin assignment attempt', payload: { providerId: provider.id, targetUserId: targetUser.id, reason: 'ADMIN_ROLE' } })
      } catch {}
      return redirectWithToast(`/admin/customer-manage/${customerId}/providers`, {
        type: 'error',
    ...
    if (!provider.providerGroupId && targetUser.providerGroupId) {
      try {
        await writeAudit({ userId, rolesCsv, customerId, action: 'PROVIDER_ASSIGN_USER_ATTEMPT', entityId: provider.id, success: false, message: 'Blocked assignment: provider ungrouped, user grouped', payload: { providerId: provider.id, targetUserId: targetUser.id, reason: 'PROVIDER_UNGROUPED_USER_GROUPED' } })
      } catch {}
      return redirectWithToast(`/admin/customer-manage/${customerId}/providers`, {
        type: 'error',
        title: 'Invalid assignment',
    ...
    if (provider.providerGroupId && provider.providerGroupId !== targetUser.providerGroupId) {
      try {
        await writeAudit({ userId, rolesCsv, customerId, action: 'PROVIDER_ASSIGN_USER_ATTEMPT', entityId: provider.id, success: false, message: 'Blocked assignment: provider grouped, user different or ungrouped', payload: { providerId: provider.id, targetUserId: targetUser.id, providerGroupId: provider.providerGroupId, userGroupId: targetUser.providerGroupId, reason: 'GROUP_MISMATCH' } })
      } catch {}
      return redirectWithToast(`/admin/customer-manage/${customerId}/providers`, {
        type: 'error',
    ...
    try {
      await writeAudit({ userId, rolesCsv, customerId, action: 'PROVIDER_ASSIGN_USER_ATTEMPT', entityId: provider.id, success: true, message: 'User assigned to provider', payload: { providerId: provider.id, targetUserId: targetUser.id } })
    } catch {}
    return redirectWithToast(`/admin/customer-manage/${customerId}/providers`, {
      type: 'success',
      title: 'User assigned',
  ...
  if (intent === 'bulk-update-user-assignments') {
    ...
    async function auditAttempt(success: boolean, payload: any, message: string) {
      try {
        await writeAudit({ userId, rolesCsv, customerId, action: 'PROVIDER_ASSIGN_USER_ATTEMPT', entityId: auditProviderId, success, message, payload })
      } catch {}
    }
    ...
    for (const uid of assignUserIds) {
      ...
      const targetUser = await prisma.user.findFirst({ where: { id: uid, customerId }, select: { id: true, name: true, email: true, providerGroupId: true, roles: { select: { name: true } } } })
      if (!targetUser) { failedSummary.push('User not found'); continue }
      if (targetUser.roles?.some(r => [INTEREX_ROLES.CUSTOMER_ADMIN, INTEREX_ROLES.PROVIDER_GROUP_ADMIN].includes(r.name))) {
...
export default function AdminCustomerProvidersPage() {
  ...
  const [drawerState, setDrawerState] = useState<{ isOpen: boolean; mode: 'create' | 'edit'; providerId?: string }>({ isOpen: false, mode: 'create' })
  ...
  const [createForm, createFields] = useForm({
    ...
    lastResult,
    onValidate({ formData }) { return parseWithZod(formData, { schema: CreateProviderSchema }) },
  })
  const [editForm, editFields] = useForm({
    id: 'edit-provider-form',
    constraint: getZodConstraint(UpdateProviderSchema),
    lastResult,
    onValidate({ formData }) { return parseWithZod(formData, { schema: UpdateProviderSchema }) },
  })

  const canAssignUsers = true
  const canToggleActive = true
  const canEditProviders = true
  const canCreateProviders = true
  ...
  return (
    <>
      ...
      <div className={`transition-all duration-300 ${drawerState.isOpen ? 'blur-sm' : 'blur-none'}`}>
        <InterexLayout
          user={user}
          title={`${customer.name} - Provider NPI Management`}
          ...
          backGuardRedirectTo={`/admin/customer-manage/${customer.id}`}
          actions={
            ...
            <div className="space-y-8">
              ...
              <div className="bg-white shadow rounded-lg p-6">
                <Form method="get" className="flex items-center space-x-4">
                  ...
                  {searchParams.search && (
                    <Link to={`/admin/customer-manage/${customer.id}/providers`} className="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">Clear</Link>
                  )}
                </Form>
              </div>
              ...
              <div className="bg-white shadow rounded-lg">
                ...
                {customer.providers.length === 0 ? (
                  <div className="px-6 py-12 text-center">
                    ...
                    <p className="text-gray-500 mb-6">{searchParams.search ? `No providers match your search criteria "${searchParams.search}".` : 'Get started by adding your first provider NPI.'}</p>
                    {canCreateProviders ? (
                      ...
                      <table className="w-full divide-y divide-gray-200 table-auto">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {customer.providers.map(provider => {
                            ...
                            const displayName =
                              (provider as any).listDetail?.providerName ||
                              (provider as any).registrationStatus?.providerName ||
                              provider.name ||
                              'No name'
                            ...
                            const eligibleUsers = (assignableUsers as any[]).filter((u: any) => {
                              ...
                              return !(provider as any).userNpis.some((l: any) => l.userId === u.id)
                            })
                            ...
                            return (
                              <tr key={provider.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap"><div className="text-sm text-gray-900">{(provider as any).providerGroup?.name || '-'}</div></td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center">
                                  {provider.userNpis.length === 0 ? (
                                    <span className="text-gray-400 text-xs">None</span>
                                  ) : (
                                    <div className="flex flex-col items-center gap-1">
                                      {(provider as any).userNpis.map((link: any) => (
                                      ...
                                      {openUserProviderId === provider.id ? (
                                        <div className="absolute z-20 -left-2 top-0 bg-white border rounded-lg shadow p-3 w-[300px]">
                                          <div className="flex items-center justify-between mb-2">
                                            ...
                                            <button type="button" onClick={() => setOpenUserProviderId(null)} className="text-gray-400 hover:text-gray-600" title="Close">
                                              <Icon name="cross-1" className="h-3 w-3" />
                                            </button>
                                          </div>
                                          {eligibleUsers.length === 0 && provider.userNpis.length === 1 ? (
                                            (() => {
                                              const singleLink = (provider as any).userNpis[0]
                                              return (
                                        ...
                                        <button type="button" disabled={!hasEligibleNewUser && provider.userNpis.length === 0} onClick={() => (hasEligibleNewUser || provider.userNpis.length > 0) ? setOpenUserProviderId(provider.id) : undefined} className={`inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium border transition-colors focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-blue-500 ${provider.userNpis.length ? 'bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100' : 'bg-emerald-50 text-emerald-700 border-emerald-200 ' + (hasEligibleNewUser ? 'hover:bg-emerald-100' : 'opacity-50 cursor-not-allowed')}`} title={!hasEligibleNewUser && provider.userNpis.length === 0 ? (provider.providerGroupId ? 'No users in this provider group available to assign.' : 'No ungrouped users available to assign.') : undefined}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-center">
                                  <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${provider.active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{provider.active ? 'Active' : 'Inactive'}</span>
                                </td>
                                      ...
                                      {openGroupProviderId === provider.id ? (
                                        <div className="absolute z-20 -left-2 top-0 bg-white border rounded-lg shadow p-3 w-64">
                                          <div className="flex items-center justify-between mb-2">
                                            ...
                                            <button type="button" onClick={() => setOpenGroupProviderId(null)} className="text-gray-400 hover:text-gray-600" title="Close">
                                              <Icon name="cross-1" className="h-3 w-3" />
                                            </button>
                                          </div>
                                        ...
                                        <button type="button" disabled={Boolean(!provider.providerGroupId && (eligibility as any)?.groupChangeBlocked)} onClick={() => !(eligibility as any)?.groupChangeBlocked ? setOpenGroupProviderId(provider.id) : undefined} className={`inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium border transition-colors focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-indigo-500 ${provider.providerGroup ? 'bg-indigo-50 text-indigo-700 border-indigo-200 hover:bg-indigo-100' : 'bg-emerald-50 text-emerald-700 border-emerald-200 ' + ((eligibility as any)?.groupChangeBlocked ? 'opacity-50 cursor-not-allowed' : 'hover:bg-emerald-100')}`} title={(eligibility as any)?.groupChangeBlocked ? (eligibility as any).groupChangeBlockReason || 'Cannot assign group due to guard rails.' : (provider.providerGroup ? 'Change provider group' : 'Assign provider group')}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                  <Form method="post" className="inline">
                                    ...
                                    <button type="submit" disabled={!canToggleActive} className={`p-1 ${provider.active ? 'text-amber-600 hover:text-amber-800' : 'text-green-600 hover:text-green-800'}`} title={provider.active ? 'Mark Inactive' : 'Activate'}>
        ...
        <Form method="post" {...getFormProps(createForm)}>
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200">
              <button type="button" onClick={closeDrawer} className="inline-flex justify-center py-2 px-4 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Cancel</button>
              <StatusButton type="button" onClick={() => (document.getElementById('create-provider-form') as HTMLFormElement | null)?.requestSubmit()} disabled={isPending} status={isPending ? 'pending' : 'idle'} className="inline-flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Create Provider NPI</StatusButton>
            </div>
          </div>
        </Form>
      </Drawer>
      ...
      <Drawer isOpen={drawerState.isOpen && drawerState.mode === 'edit'} onClose={closeDrawer} title={`Edit NPI ${selectedProvider?.npi || 'Provider'}`} size="md">
        {selectedProvider && (
          <Form method="post" {...getFormProps(editForm)}>
            <input type="hidden" name="intent" value="update" />
            <input type="hidden" name="providerId" value={selectedProvider.id} />
            <div className="space-y-6">
              <Field labelProps={{ children: 'National Provider Identifier (NPI) - Read Only' }} inputProps={{ type: 'text', value: selectedProvider.npi, disabled: true, className: 'bg-gray-50 text-gray-500' }} />
              <Field labelProps={{ children: 'Provider Name *' }} inputProps={{ ...getInputProps(editFields.name!, { type: 'text' }), defaultValue: selectedProvider.name || '', placeholder: 'e.g., Dr. John Smith' }} errors={editFields.name?.errors} />
              {(selectedProvider as any).listDetail?.providerName || (selectedProvider as any).registrationStatus?.providerName ? (
                <div className="text-xs text-gray-500 -mt-3">Authoritative (PCG): {(selectedProvider as any).listDetail?.providerName || (selectedProvider as any).registrationStatus?.providerName}</div>
              ) : null}
              <SelectField labelProps={{ children: 'Provider Group (Optional)' }} selectProps={{ ...getInputProps(editFields.providerGroupId!, { type: 'text' }), defaultValue: (selectedProvider as any).providerGroupId || '' }} errors={editFields.providerGroupId?.errors}>
                <option value="">— No group —</option>
                {customer.providerGroups.map(group => (<option key={group.id} value={group.id}>🏥 {group.name}</option>))}
              </SelectField>
              <div>
                <label className="flex items-center">
                  ...
                  <input {...getInputProps(editFields.active!, { type: 'checkbox' })} value="true" defaultChecked={selectedProvider.active} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" />
              ...
              <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200">
                <button type="button" onClick={closeDrawer} className="inline-flex justify-center py-2 px-4 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Cancel</button>
                <StatusButton type="button" onClick={() => (document.getElementById('edit-provider-form') as HTMLFormElement | null)?.requestSubmit()} disabled={isPending} status={isPending ? 'pending' : 'idle'} className="inline-flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Save Changes</StatusButton>
              </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-3">
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className="text-lg font-bold text-blue-600">{(selectedProvider as any)._count.userNpis}</p>
                      </div>
                    </div>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-3">
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className={`text-lg font-bold ${selectedProvider.active ? 'text-green-600' : 'text-red-600'}`}>{selectedProvider.active ? 'Active' : 'Inactive'}</p>
                      </div>
                    </div>
                  </div>
                </div>
