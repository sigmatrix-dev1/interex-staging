...
export default function Submissions() {
  ...
  const getStageColor = (stage: string, hasErrorEvent: boolean) => {
    ...
    if (s.includes('draft')) return 'bg-gray-100 text-gray-800'          // Draft is always gray
    if (hasErrorEvent) return 'bg-red-100 text-red-800'                   // non‑Draft + error -> red
  ...
  // Pull the most recent PCG_STATUS payload (if any)
                    ...
                    <div className="overflow-x-auto shadow-sm">
                      <div className="inline-block min-w-full align-middle">
                        <table className="min-w-full divide-y divide-gray-200 table-fixed">
                          <colgroup>
                            <col className="w-[28ch]"/>    {/* Title */}
                            <col className="w-[18ch]"/>    {/* Purpose */}
                            <col className="w-[22ch]"/>    {/* NPI */}
                            <col className="w-[40ch]"/>    {/* Stage */}
                            <col className="w-[12ch]"/>    {/* Documents */}
                            <col className="w-[16ch]"/>    {/* Created */}
                            <col className="w-[12ch]"/>    {/* Actions */}
                          </colgroup>
                          ...
                          {formatSubmissionPurpose(s.purposeOfSubmission)}
                                ...
                                <td className="px-4 py-4 text-right w-20">
                                  <div className="flex items-center justify-end space-x-2">
                                    ...
                                    {/* Decide whether to show Continue or Retry */}
                                    {(() => {
                                      const stage = latestStageFromPCG(s).toLowerCase()
                                      const isDraft = stage.includes('draft') // only Draft shows actions
                                      ...
                                      if (!hasErrorEvent) {
                                        // Clean Draft path -> Continue to Step 2 (Review)
                                        return (
                                            <Link
                                      ...
                                      // Draft + has at least one error -> Retry (server chooses the right step)
                                      return (
                                          <Form method="POST">
          ...
          {drawerState.selectedSubmission ? (
              <div className="space-y-6">
                ...
                {(() => {
                  ...
                  // transaction ids: prefer latest PCG status payload (transactionIdList, esmdTransactionId),
                  // fallback to the DB's single transactionId column.
                      ...
                      <>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-900">
                            ...
                            {intended?.oid ? (
                                <span
                                    className="ml-2 inline-block rounded bg-gray-100 px-2 py-0.5 text-xs font-mono text-gray-700"> {intended.oid}
                                </span>
                            ) : null}
                          </p>
                        </div>
                              ...
                              <ul className="mt-1 text-sm text-gray-900 list-disc list-inside space-y-1">
                                {docSet.map((d, i) => (
                                    <li key={`${i}-${d.filename || d.name || 'doc'}`}>
                                      ...
                                      {d.split_no ? <span
                                          className="ml-2 text-xs text-gray-500">split #{d.split_no}</span> : null}
                                      {d.attachmentControlNum ? (
                                          <span
                                              className="ml-2 text-xs text-gray-500">ACN {d.attachmentControlNum}</span>
                                      ) : null}
                                    </li>
                                ))}
                              </ul>
                              ...
                              <div className="mt-2 overflow-hidden rounded border border-gray-200">
                                <table className="min-w-full divide-y divide-gray-200">
                                  ...
                                  <tr>
                                    ...
                                    <th className="px-3 py-2 text-left text-xs font-medium bold text-gray-900 tracking-wider">esMD Txn
                                      ID
                                    </th>
